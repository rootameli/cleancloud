...
Pour activer l'exécution des modules externes (Modules_CleanCloud), définissez `LAB_MODE=1` avant de lancer l'application ou les tests. Exemple :

```
LAB_MODE=1 uvicorn app.main:app --reload
```

Les modules sont déclenchés en sous-processus et leurs logs sont persistés en base pour consultation via l'API ou le tableau de bord.
