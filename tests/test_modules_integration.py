...
def test_registry_contains_module():
    assert get_module("leancloud_js") is not None


def test_runner_persists_events(monkeypatch, setup_db, tmp_path):
    ...
    MODULES_REGISTRY[test_module_id] = ModuleDefinition(
        ...
        entrypoint=["python", "-c", "print('hello'); import sys; print('oops', file=sys.stderr)"],
        timeout=5,
        output_paths={},
    )
    ...
    asyncio.run(run_module(str(scan_uuid), test_module_id, ["http://example.com"], {}))

    async def _fetch():
        async with session_factory() as session:
            return (await session.execute(
                select(EventDB).where(EventDB.scan_id == scan_uuid)
            )).scalars().all()
...
def test_parser_creates_findings(tmp_path, setup_db):
    session_factory = setup_db
    ...
    asyncio.run(_prepare())
    findings = asyncio.run(parse_leancloud_js(str(scan_uuid), "leancloud_js", {"result_dir": str(result_dir)}))
    ...
    assert findings
    assert stored
    assert stored[0].evidence == "secret"
